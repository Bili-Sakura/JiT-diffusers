...
def _infer_model_type_from_config(model_config) -> str | None:
    ...
    for model_type, preset in JIT_PRESET_CONFIGS.items():
        if (
...
def main():
    ...
    if model_type is None:
        raise ValueError(
            "Could not infer JiT model_type from config. "
            "Pass --metadata_path generated by convert_jit_to_diffusers.py."
        )
